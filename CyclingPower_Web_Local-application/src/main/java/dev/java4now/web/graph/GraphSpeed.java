...
public class GraphSpeed {

    public static void draw_speed(double width, double height, Canvas layer_1, Canvas layer_2, GraphicsContext context1,GraphicsContext context2){
...
//            txt = Format.formatDouble_GWT(km_seq * i,1);
        ...
        // samo horizontalne linije po vertikali
        for (int i = 1; i < 5; i++) {
...
//--------------------- GRAPH  ----------------------
        ...
        double speed_scale;
        if( max_speed > 0 ){
            speed_scale = (height - 40) / max_speed;
        }else {
            speed_scale = 0;
        }
...
//			context1.arc(one_hour_width * i, max_pos - (int)y * scale_temp,2 ,0 ,2 * Math.PI );
        ...
        context1.closePath();
        context1.beginPath();
        context1.setStroke(Color.rgb(2, 2, 100)); // green
        context1.setLineWidth(2);
        for (int i = 0; i < width; i++) {
            double y = activity.getRecords().get((int) (graph_step * i)).getSpeed();
            if (i == 0) {
                context1.moveTo(start_position_x, height-40/*start_position_y*/ - (y * speed_scale));    // TODO - visina graph-a  veca nego sto treba
                continue;
            }
            context1.lineTo(start_position_x + i, height-40/*start_position_y*/ - (y * speed_scale));
            if (i == width - 1) {
                context1.lineTo(width, height-40/*start_position_y*/ - (activity.getRecords().get(activity.getRecords().size() - 1).getSpeed() * speed_scale));
            }
        }
        context1.stroke();
...
// zatvorena putanja koja pocinje od beginPath , nema stroke za deo putanje koji se ne vidi pa ne treba setGlobalAlpha(0);
        ...
        context1.setFill(Color.rgb(2, 2, 100)); // green
        context1.fill();
        ...
        // ispis vertikale ( samo tekst ) da bi bio iznad grafa kada je sa unutrasnje strane < 760
        //---------------------------------------------------
        if (width < 1080) {
        ...
        double max_spd_tmp = ( max_speed  * 3.6 ) * 0.82;
        double step_spd = max_spd_tmp / 4;
        for (int i = 1; i < 5; i++) {
            double current_pos = start_position_y - (step_horizontale * i);
//            txt = (int)(step_alt * i) + " m";
            if(i == 4){
                txt = Format.formatDouble_GWT((step_spd * i)*1,0) + " Km/h";
            }else{
                txt = Format.formatDouble_GWT((step_spd * i),0) + " Km/h";
            }
            if (width < 1080) {
...
//                context1.setFill(Color.rgb(150, 150, 150));
    ...
    //----------------------------------------------------------------
    public static void handle_speed(Canvas layer_2,MouseEvent event){
        if(activity == null){
            return;
        }
        ...
        context2.beginPath();
        // fill box background
        context2.setFill(Color.rgb(2, 2, 100));
        ...
        var speed = activity.getRecords().get((int) (graph_step * pos)).getSpeed()*3.6;
// Formatiranje na 1 decimalu
        String speed_str = Format.formatDouble_GWT(speed,1);
        context2.fillText("Speed: " + speed_str + " km/h",x+2,layer_2.getLayoutY()+20 + 2 + font.getSize());
        if(have_cadence.get()){
            context2.fillText("Power: " + activity.getRecords().get((int) (graph_step * pos)).getPower() + " W",x+2,layer_2.getLayoutY()+20 + 2 + (font.getSize() * 2));
            context2.fillText("Cadence: " + activity.getRecords().get((int) (graph_step * pos)).getCadence() + " rpm", x + 2, layer_2.getLayoutY() + 20 + 2 + (font.getSize() * 3));
        }else{
...
//                Console.log("getX: " + ( event.getX() - start_position_x));
    ...
    //--------------------------------------------------------------
    public static void handle_slider_speed(Slider slider, Number newValue, Canvas layer_2){
...
// TODO samo pos umesto (pos-start_position_x) daje tacan raspon na mapi ali ispis ne odgovara
        ...
        context2.fillText(str, x + 2, 2 + font.getSize());
        context2.setFill(Color.rgb(255, 255, 255));
        var speed = activity.getRecords().get((int) (graph_step * (pos-start_position_x)) - 1).getSpeed() * 3.6;
        // Formatiranje na 1 decimalu
        String speed_str = Format.formatDouble_GWT(speed,1);
        context2.fillText("Speed: " + speed_str + " km/h", x + 2, layer_2.getLayoutY() + 20 + 2 + font.getSize());
        if(have_cadence.get()){
            context2.fillText("Power: " + activity.getRecords().get((int) (graph_step * (pos-start_position_x)) - 1).getPower() + " W", x + 2, layer_2.getLayoutY() + 20 + 2 + (font.getSize() * 2));
            context2.fillText("Cadence: " + activity.getRecords().get((int) (graph_step * (pos-start_position_x)) - 1).getCadence() + " rpm", x + 2, layer_2.getLayoutY() + 20 + 2 + (font.getSize() * 3));
        }else{
