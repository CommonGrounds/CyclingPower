...
public class GraphTemperature {
    ...
    public static void draw_temperature(double width, double height, Canvas layer_1, Canvas layer_2, GraphicsContext context1, GraphicsContext context2){
...
//--------------------- GRAPH  ----------------------
        ...
        if( max_temperature > 0 ){
            temp_scale = (height - 40) / ( max_temperature * 1.2 );
        }else {
            temp_scale = 0;
        }
        Console.log("max_temperature: " + max_temperature);
        // --- Altitude -----
...
//			context1.arc(one_hour_width * i, max_pos - (int)y * scale_temp,2 ,0 ,2 * Math.PI );
        ...
        context1.closePath();
        context1.beginPath();
        context1.setStroke(Color.rgb(0, 0, 0)); // green
        context1.setLineWidth(2);
        for (int i = 0; i < width; i++) {
            double y = activity.getRecords().get((int) (graph_step * i)).getTemperature();
            if (i == 0) {
                context1.moveTo(start_position_x, height-40/*start_position_y*/ - (y * temp_scale));    // TODO - visina graph-a  veca nego sto treba
                continue;
            }
            context1.lineTo(start_position_x + i, height-40/*start_position_y*/ - (y * temp_scale));
            if (i == width - 1) {
                context1.lineTo(width, height-40/*start_position_y*/ - (activity.getRecords().get(activity.getRecords().size() - 1).getTemperature() * temp_scale));
            }
        }
        context1.stroke();
...
// zatvorena putanja koja pocinje od beginPath , nema stroke za deo putanje koji se ne vidi pa ne treba setGlobalAlpha(0);
        ...
        context1.setTextAlign(TextAlignment.RIGHT);
        String txt2 = "Temp";
        context1.fillText(txt2,width - Service_impl.measureText(txt2, layer_2),start_position_y - 10);
        context1.setGlobalAlpha(0.3);
        context1.setFill(Color.rgb(163, 151, 77)); // green
        context1.fill();
    ...
    //----------------------------------------------------------------
    public static void handle_temperature(Canvas layer_2, MouseEvent event){
        if(activity == null){
            return;
        }
        ...
        context2.setFill(Color.rgb(0, 0, 0));
        var temperature = activity.getRecords().get((int) (graph_step * pos)).getTemperature();
        context2.fillText("Temp: " + temperature + " C°",x+2,layer_2.getLayoutY()+20 + 2 + font.getSize());
...
//                Console.log("getX: " + ( event.getX() - start_position_x));
    ...
    //--------------------------------------------------------------
    public static void handle_slider_temperature(Slider slider, Number newValue, Canvas layer_2){
...
// TODO samo pos umesto (pos-start_position_x) daje tacan raspon na mapi ali ispis ne odgovara
        ...
        context2.fillText(str, x + 2, 2 + font.getSize());
        context2.setFill(Color.rgb(0, 0, 0));
        var temperature = activity.getRecords().get((int) (graph_step * (pos-start_position_x)) - 1).getTemperature();
        context2.fillText("Temp: " + temperature + " C°", x + 2, layer_2.getLayoutY() + 20 + 2 + font.getSize());
