...
public class GraphPower {
    ...
    public static void draw_power(double width, double height, Canvas layer_1, Canvas layer_2, GraphicsContext context1, GraphicsContext context2){
...
//--------------------- GRAPH  ----------------------
        ...
        double power_scale;
        if( max_power > 0 ){
            power_scale = (height - 40) / max_power;
        }else {
            power_scale = 0;
        }
        Console.log("max_power: " + max_power);
        // --- Altitude -----
...
//			context1.arc(one_hour_width * i, max_pos - (int)y * scale_temp,2 ,0 ,2 * Math.PI );
        ...
        context1.closePath();
        context1.beginPath();
        context1.setStroke(Color.rgb(100, 2, 2)); // green
        context1.setLineWidth(2);
        for (int i = 0; i < width; i++) {
            double y = activity.getRecords().get((int) (graph_step * i)).getPower();
            if (i == 0) {
                context1.moveTo(start_position_x, height-40/*start_position_y*/ - (y * power_scale));    // TODO - visina graph-a  veca nego sto treba
                continue;
            }
            context1.lineTo(start_position_x + i, height-40/*start_position_y*/ - (y * power_scale));
            if (i == width - 1) {
                context1.lineTo(width, height-40/*start_position_y*/ - (activity.getRecords().get(activity.getRecords().size() - 1).getPower() * power_scale));
            }
        }
        context1.stroke();
...
// zatvorena putanja koja pocinje od beginPath , nema stroke za deo putanje koji se ne vidi pa ne treba setGlobalAlpha(0);
    ...
    //----------------------------------------------------------------
    public static void handle_power(Canvas layer_2, MouseEvent event){
        if(activity == null){
            return;
        }
        ...
        context2.beginPath();
        // fill box background
        context2.setFill(Color.rgb(100, 2, 2));
        ...
        var power = activity.getRecords().get((int) (graph_step * pos)).getPower();
        context2.fillText("Power: " + power + " W",x+2,layer_2.getLayoutY()+20 + 2 + font.getSize());
...
//                Console.log("getX: " + ( event.getX() - start_position_x));
    ...
    //--------------------------------------------------------------
    public static void handle_slider_power(Slider slider, Number newValue, Canvas layer_2){
...
// TODO samo pos umesto (pos-start_position_x) daje tacan raspon na mapi ali ispis ne odgovara
        try {
            ...
            context2.setFill(Color.rgb(255, 255, 255));
//            var power = activity.getRecords().get((int) (graph_step * pos)).getPower();
            var power = activity.getRecords().get((int) (graph_step * (pos-start_position_x)) - 1).getPower();
            context2.fillText("Power: " + power + " W", x + 2, layer_2.getLayoutY() + 20 + 2 + font.getSize());
            ...
            context2.setGlobalAlpha(1);
                ...
                Console.log("GraphPower value: " + ((graph_step * (pos-start_position_x))));
                context2.clearRect(0, 0, layer_2.getWidth(), layer_2.getHeight());
                return;
            }
